          ...
          <div className="p-3 bg-white flex flex-col gap-3 max-h-60 overflow-y-auto border-t border-gray-100">
            {metadata.steps.map((step, idx) => (
              ...
              >
                ...
                <div className="text-gray-500 bg-gray-50 p-1.5 rounded-md font-mono text-[10px]">
                  Input: {JSON.stringify(step.tool_input)}
                </div>
                <div
                  className="text-gray-600 bg-blue-50/50 p-1.5 rounded-md truncate"
        ...
        <details className="mt-1 text-xs text-gray-600 border border-gray-200 rounded-xl overflow-hidden w-full bg-white shadow-sm hover:shadow transition-shadow">
          ...
          <div className="p-3 bg-white flex flex-col gap-3 max-h-60 overflow-y-auto border-t border-gray-100">
            {metadata.sources.map((source, idx) => (
              <div
                key={idx}
                className="flex flex-col gap-1 border border-gray-100 rounded-lg p-2.5 bg-gray-50"
              >
                <div
                  className="font-semibold text-gray-800 truncate"
                  title={source.metadata?.source || "Unknown file"}
                >
                  📄{" "}
                ...
                <div className="text-gray-500 line-clamp-3 leading-relaxed mt-1">
                  {source.content}
                </div>
              </div>
            ))}

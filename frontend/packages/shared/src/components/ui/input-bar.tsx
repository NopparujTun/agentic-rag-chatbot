...
function AttachmentButton({ onClick, disabled }: { onClick: () => void; disabled?: boolean }) {
  return (
    <button
      ...
      disabled={disabled}
      aria-label="Attach"
      className="inline-flex items-center justify-center w-8 h-8 rounded-full text-neutral-500 hover:bg-neutral-100:bg-neutral-800 transition-colors disabled:opacity-40"
    >
      <PaperclipIcon />
    </button>
  );
...
function FileChip({
  ...
  return (
    <div className="inline-flex items-center gap-2 px-2 py-1.5 rounded-md bg-neutral-100 group">
      ...
      {onRemove && (
        <button
          ...
          className="inline-flex items-center justify-center w-5 h-5 rounded-full text-neutral-500 hover:bg-neutral-200:bg-neutral-700 opacity-0 group-hover:opacity-100 transition-opacity"
        >
          <XIcon />
        </button>
      )}
  ...
  return (
    <div className={cn("shrink-0 px-3 pb-3 w-full", className)}>
      <div className="mx-auto max-w-full w-full">
        <div
          className="relative cursor-text rounded-[16px] bg-white shadow-sm ring-1 ring-neutral-200"

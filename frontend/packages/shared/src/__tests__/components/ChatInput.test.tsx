...
vi.mock('../../components/ui/input-bar', () => ({
  InputBar: ({ onSend, onAttach, status }: any) => (
    <div data-testid="input-bar" data-status={status}>
      <button data-testid="send-btn" onClick={() => onSend({ role: 'user', content: 'test message' })}>Send</button>
      <button data-testid="attach-btn" onClick={onAttach}>Attach</button>
    </div>
  )
}));
...
describe('ChatInput', () => {
  ...
  it('calls onUploadDocuments when files are selected', () => {
    ...
    render(<ChatInput isSending={false} onSendMessage={() => {}} onUploadDocuments={onUploadMock} />);
